...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( 'Curti', 'hFansite' ),
		'unlike' => __( 'Descurtir', 'hFansite' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Não é permitido', 'hFansite' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Descurtir', 'hFansite' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Curtir', 'hFansite' );
		$icon = $icon_empty;
	}
...
function show_user_likes( $user ) { ?>        
	<label for="user_likes"><?php _e( 'Você curtiu:', 'hFansite' ); ?></label>
	<div class="card">
		<div class="card-body">
			<?php
				...
				<?php while ( $like_query->have_posts() ) : $like_query->the_post(); 
					echo $sep; ?><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a><hr>
			<?php
				endwhile; 
			?>
		</div>
	...
	<?php else : ?>
	<p><?php _e( 'Você ainda não curtiu nada.', 'hFansite' ); ?></p>
	<?php 
	endif; 
	wp_reset_postdata(); 
	?>
