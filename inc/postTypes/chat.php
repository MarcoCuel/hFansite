...
class Chatroom {
	...
	function __construct() {
		...
		load_plugin_textdomain( 'chat', false, 'chat/languages' );
		...
		add_action( 'publish_chat', array( $this, 'yoast_fix' ), 10, 2 );
	}
	...
	function register_post_types() {
		$labels = array(
			...
			'add_new_item'       => ( 'Adicionar novo chat'),
			'new_item'           => ( 'Novo chat'),
			'edit_item'          => ( 'Editar chat'),
			'view_item'          => ( 'Ver chat'),
			'all_items'          => ( 'Todos os chats'),
	...
	function enqueue_scripts() {
		...
		if ( 'chat' !== $post->post_type ) {
			return;
		}

		wp_enqueue_script( 'chat', get_template_directory_uri() . '/assets/js/chat.js', array('jquery'), '1.0', true );
	}
	...
	function maybe_create_chatroom_log_file( $post_id, $post ) {
		...
		if ( empty( $post->post_type ) || 'chat' !== $post->post_type ) {
			return;
		}
	...
	function the_content_filter( $content ) {
		...
		if ( '1' === get_post_meta( $post->ID, '_logged_in_only', true ) ) {
			echo apply_filters( 'chat_room_not_logged_in_msg', __( 'Você precisa estar logado para participar do chat.', 'chat' ) );
			return;
		}
		...
		<textarea class="chat-text-entry" placeholder="Digite uma mensagem..."></textarea>
		<?php
	...
	function register_meta_box() {
		...
		foreach ( $screens as $screen ) {
			add_meta_box( 'logged-in-only', esc_html_e( 'Privacidade?', 'chat' ), array( $this, 'render_meta_box' ), $screen, 'side', 'default' );
		}
	}
	...
	function render_meta_box( $post ) {
		...
		<label for="logged_in_only"><?php esc_html_e( 'O usuário deve estar logado?', 'chat' ); ?></label><br/>
		<?php
	}
	...
	function usability_notes() {
		printf(
			'<p><small>%s</small></p>',
			esc_html__( 'Pressione enter ou clique em "Enviar" para enviar. Clique em um nome de usuário para adicioná-lo à área de texto.', 'chat' )
		);
	}
